				...
				if (!bol.getStatus().equalsIgnoreCase(Constants.BOL_STATUS.REJECTED.value)) {
					for (DoAuthRequest doAuthRequestEntity : bol.getDoAuthRequests()) {
						...
						if (!doAuthRequestEntity.getApprovalLog().isEmpty()) {
							List<ApprovalLog> approvalLog1 = doAuthRequestEntity.getApprovalLog().stream().sorted(Comparator.comparing(ApprovalLog::getCreatedDate).reversed()).collect(Collectors.toList());
							ApprovalLog approvalLog = approvalLog1.iterator().next();
