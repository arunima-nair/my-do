...
import { MonitorModelComponent } from './monitor-model/monitor-model.component';
...
@Component({
  selector: 'app-monitor-file-uploads',
  templateUrl: './monitor-file-uploads.component.html',
  styleUrls: ['./monitor-file-uploads.component.css']
})

export class MonitorFileUploadsComponent extends BaseComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.userType === 'A') {
      this._httpRequestService.getData('/do/app/api/secure/getAgentDetailByUserDTO', false).subscribe((res) => {
        ...
        this.monitorForm.get('shippingAgentName').setValue(res[0].label);
      });
    } else
      this.shippingAgent = "";
    if (this.userType === 'I') {
  ...
  editFL(event) {
    ...
    this._dialogService
      .openDialog(MonitorModelComponent, data, dialogOptions)
      .subscribe(retval => {
        this.shippingAgentCode = this.shippingAgentCode.split('-')[0];
      });
    //this.shippingAgentCode = this.shippingAgentCode.split('-')[0];
